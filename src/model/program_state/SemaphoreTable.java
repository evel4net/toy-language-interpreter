...
public class SemaphoreTable {
    private Map<Integer, Tuple<Integer, ArrayList<Integer>, Integer>> semaphoreTable = new ConcurrentHashMap<>();
    private int newFreeAddress = 1;

    public synchronized int addNewSemaphore(int value1, int value2) throws KeyAlreadyExistsException {
        ...
        this.semaphoreTable.put(entryAddress, new Tuple<>(value1, new ArrayList<>(), value2));
        this.newFreeAddress += 1;

        return entryAddress;
    }

    public synchronized Tuple<Integer, ArrayList<Integer>, Integer> getTuple(int address) throws KeyNotDefinedException {
    ...
    public synchronized Map<Integer, Tuple<Integer, ArrayList<Integer>, Integer>> getContent() {
        return Map.copyOf(this.semaphoreTable);
    }

    public synchronized void updateEntry(int address, Tuple<Integer, ArrayList<Integer>, Integer> newTuple) throws KeyNotDefinedException {
        if (!this.existsAddress(address)) throw new KeyNotDefinedException(Integer.toString(address));

        this.semaphoreTable.put(address, newTuple);
    }
