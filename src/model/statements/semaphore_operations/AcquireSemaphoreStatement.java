...
public class AcquireSemaphoreStatement implements Statement {
    ...
    public ProgramState execute(ProgramState state) throws ProgramException {
        ...
        int currentNumberThreads = currentThreads.size();
        int totalNumberThreads = semaphoreContent.getKey();

        if (totalNumberThreads > currentNumberThreads) {
            if (!currentThreads.contains(state.getId())) {
                currentThreads.add(state.getId());
    ...
    public Statement deepCopy() {
        return new AcquireSemaphoreStatement(this.variable);
    }
